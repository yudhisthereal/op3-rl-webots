...
from controllers.op3_ppo_env.op3_ppo_env import PPOAgent
...
if __name__ == "__main__":
    robot = Supervisor()
    scenario = SCENARIO_CLASS(robot, timestep=config.TIMESTEP, algorithm="ppo")
    timestep = int(robot.getBasicTimeStep())
    
    print("=" * 70)
    print("🤖 PPO AGENT TESTING")
    print("=" * 70)
    
    # Create scenario (only once!)
    ...
    if robot.step(timestep) == -1:
        print("❌ Simulation ended before starting")
        robot.simulationQuit(1)
    ...
    while robot.step(timestep) != -1 and step < config.MAX_STEPS:
